...
- Ordinary sessions mount complete live host roots read-write. Never split their children into Cooper-owned state. Runtime mounts preserve public targets. Named sessions mount only the exact registered roots of their selected profile. Never expose a profile parent, recovery directory, or another account through a state or workspace mount. Apply read-only Git hook limits through both the workspace and public state paths.
- Account profiles are Linux-only. macOS profiles are disabled. There are no upgrade paths for the old copy or symlink formats; reject them without changing their data. Build and configure's Save & Build must not set up, move, or convert profiles. First save registers Default in place. Inactive complete roots use stable-ID siblings of their public paths. Load uses checked same-parent renames, not copies or links. Standalone files follow the same rename journal; optional file absence is preserved. Normal save/load uses bounded metadata and never scans history. The `.agents` root stays global and is mounted from the host; it is not part of an account profile. Load an empty profile before changing accounts and refuse mapped account mismatches. Keep backup, restore, deletion, and recovery cleanup explicit. Confirm that affected CLIs, apps, and runtimes are stopped before a host switch. Prompts run outside the state lock; recheck under the lock after confirmation. Confirmation never overrides known use. Recovery uses exact parent and entry identities and must not overwrite unexpected data. Restore keeps source paths fixed. Preserve profile roots and metadata during runtime/cache cleanup. See [cooper/TECHNICAL.md](cooper/TECHNICAL.md#profiles) for identity, recovery, and credential rules.
...
- Reject a Grok state root that overlaps the Cooper directory in either direction. Check the direct paths and the paths after existing symlinks are resolved. Named sources also require exact registered profile-root validation.
...
- **Cooper VM development:** Run `./cooper/test-vm-dev.sh unit` first. It
  ...
  [cooper/TECHNICAL.md](cooper/TECHNICAL.md#prepared-vm-development) for cache ownership, reports, and command limits.
