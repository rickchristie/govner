...
func TestRenderBaseDockerfile_DefaultConfig(t *testing.T) {
	...
	assertContains(t, result, "/usr/local/libexec/cooper-setup-account")
	...
	assertContains(t, result, "ENV PATH=/opt/cooper/bin:/opt/cooper/npm/bin:/opt/cooper/python/bin:"+docker.BarrelGoBinDir+":$PATH")
...
func TestRenderBaseDockerfile_ImplicitToolsAppearBeforeRuntimeProxyEnv(t *testing.T) {
	...
	userIdx := strings.Index(result, "USER ${USER_NAME}")
	...
	if userIdx == -1 || goplsIdx == -1 || proxyIdx == -1 {
		t.Fatalf("expected USER ${USER_NAME}, gopls install, and proxy env sections in Dockerfile")
	}
...
func TestRenderCLIToolDockerfile_Claude(t *testing.T) {
	...
	assertNotContains(t, result, "/opt/cooper/bin/claude install")
...
func TestRenderCLIToolDockerfile_OpenCode(t *testing.T) {
	...
	assertContains(t, result, "/opt/cooper/bin/opencode")
	assertContains(t, result, "COOPER_CLI_TOOL=opencode")
	assertContains(t, result, "$HOME/.config/opencode")
	assertContains(t, result, "$HOME/.local/share/opencode")
	assertContains(t, result, "$HOME/.local/state/opencode")
	assertContains(t, result, "$HOME/.opencode")
...
func TestRenderCLIToolDockerfile_OpenCodeVersionPinned(t *testing.T) {
	...
	assertContains(t, result, "cp \"$src\" /opt/cooper/bin/opencode")
	assertNotContains(t, result, "opencode.ai/install")
...
func TestRenderCLIToolDockerfile_Grok(t *testing.T) {
	...
	assertContains(t, result, "/opt/cooper/bin/grok")
	assertContains(t, result, "chmod 0755 /opt/cooper/bin/grok")
	assertContains(t, result, "case \"${TARGETARCH:-}\"")
	...
	assertContains(t, result, "USER ${COOPER_USER_NAME}")
	assertNotContains(t, result, "ENV GROK_HOME=")
	...
	assertContains(t, result, "$HOME/.grok")
...
func TestRenderEntrypoint_UnifiedXvfb(t *testing.T) {
	...
	assertContains(t, result, docker.BarrelPlaywrightCacheDir)
