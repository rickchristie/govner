...
run_build_test() {
    ...
    if docker image inspect "$grok_image" &>/dev/null; then
        pass "${mode}: grok image '${grok_image}' exists"
    else
        fail "${mode}: grok image '${grok_image}' not found"
    fi
    ...
    if [ -f "${test_dir}/cli/grok/Dockerfile" ]; then
        pass "${mode}: grok Dockerfile generated"
    else
        fail "${mode}: grok Dockerfile not found"
    fi
    if grep -q "# Cooper CLI: Grok Build - Generated by cooper configure" "${test_dir}/cli/grok/Dockerfile"; then
        pass "${mode}: grok Dockerfile has generated marker"
    else
        fail "${mode}: grok Dockerfile missing generated marker"
    fi
    if [ ! -e "${test_dir}/cli/grok/requirements.toml" ]; then
        pass "${mode}: grok does not generate a host-setting override"
    else
        fail "${mode}: obsolete grok requirements.toml was generated"
    fi
    if grep -q "npm install" "${test_dir}/cli/grok/Dockerfile" || grep -q "install.sh" "${test_dir}/cli/grok/Dockerfile"; then
        fail "${mode}: grok Dockerfile uses npm or moving installer"
    else
        pass "${mode}: grok Dockerfile uses official binary artifact"
    fi
    if grep -q "opencode.ai/install" "${test_dir}/cli/opencode/Dockerfile"; then
        fail "${mode}: opencode Dockerfile uses convenience installer"
    elif grep -q "github.com/anomalyco/opencode/releases/download/" "${test_dir}/cli/opencode/Dockerfile"; then
        pass "${mode}: opencode Dockerfile uses official GitHub release artifact"
    else
        fail "${mode}: opencode Dockerfile missing official GitHub release artifact"
    fi
    ...
    if base_run which grok &>/dev/null; then
        fail "${mode}: base image has 'grok' (should not)"
    else
        pass "${mode}: base image does not have 'grok'"
    fi
    ...
    grok_run() {
        docker run --rm --entrypoint "" "$grok_image" "$@" 2>&1
    }
    ...
    local grok_version
    grok_version=$(get_tool_version ai_tools grok)
    if [ "$(is_tool_enabled ai_tools grok)" = "true" ]; then
        local actual_grok
        actual_grok=$(grok_run bash -c 'grok --version 2>&1' || true)
        assert_version "Grok Build" "$grok_version" "$actual_grok"
        ...
        chmod 755 "${grok_state_dir}/bin/grok"
        printf '%s\n' '[memory]' 'enabled = true' > "${grok_state_dir}/config.toml"
        ...
        if grep -q barrel-write "${grok_state_dir}/from-barrel"; then
            ...
            fail "${mode}: grok complete state mount did not persist a write"
        fi
    fi
    if grok_run which claude &>/dev/null; then
        fail "${mode}: grok image has 'claude' (should not)"
    else
        pass "${mode}: grok image does not have 'claude'"
    fi
    if claude_run which grok &>/dev/null; then
        fail "${mode}: claude image has 'grok' (should not)"
    else
        pass "${mode}: claude image does not have 'grok'"
    fi
    local grok_cli_tool
    grok_cli_tool=$(grok_run bash -c 'echo $COOPER_CLI_TOOL' || true)
    if [ "$grok_cli_tool" = "grok" ]; then
        pass "${mode}: grok image COOPER_CLI_TOOL=grok"
    else
        fail "${mode}: grok image COOPER_CLI_TOOL='${grok_cli_tool}' (expected 'grok')"
    fi
    local grok_auto
    grok_auto=$(grok_run bash -c 'echo $COOPER_CLI_AUTO_APPROVE' || true)
    if [ "$grok_auto" = "--always-approve" ]; then
        pass "${mode}: grok image COOPER_CLI_AUTO_APPROVE is exact"
    else
        fail "${mode}: grok image COOPER_CLI_AUTO_APPROVE='${grok_auto}'"
    fi
    local grok_clip
    grok_clip=$(grok_run bash -c 'echo $COOPER_CLIPBOARD_MODE' || true)
    if [ "$grok_clip" = "x11" ]; then
        pass "${mode}: grok image COOPER_CLIPBOARD_MODE=x11"
    else
        fail "${mode}: grok image COOPER_CLIPBOARD_MODE='${grok_clip}'"
    fi
    local grok_home
    ...
    local grok_leader_socket
    grok_leader_socket=$(grok_run bash -c 'echo $GROK_LEADER_SOCKET' || true)
    if [ "$grok_leader_socket" = "/tmp/cooper-grok-leader.sock" ]; then
        pass "${mode}: grok leader transport uses the barrel tmp path"
    else
        fail "${mode}: grok image GROK_LEADER_SOCKET='${grok_leader_socket}'"
    fi
    local grok_policy_env
    grok_policy_env=$(grok_run env || true)
    if [ -e "${test_dir}/cli/grok/requirements.toml" ] || grok_run test -e /etc/grok/requirements.toml || echo "$grok_policy_env" | grep -qE '^GROK_(AUTH_PATH|MEMORY|WEB_FETCH|DISABLE_AUTOUPDATER|DISABLE_API_KEY_AUTH)='; then
        fail "${mode}: grok image overrides host settings"
    else
        pass "${mode}: grok image does not override host settings"
    fi
